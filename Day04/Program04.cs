...
namespace Day04
{
    class PassportData
    {
        public string BirthYear { get; } = null;
        public string Country { get; } = null;
        public string ExpiryYear { get; } = null;
        public string EyeColor { get; } = null;
        public string HairColor { get; } = null;
        public string Height { get; } = null;
        public string Id { get; } = null;
        public string IssueYear { get; } = null;

        public PassportData(in IEnumerable<string> data)
        {
            foreach (var d in data)
            {
                ...
                if (string.IsNullOrWhiteSpace(t))
                    ...
                    throw new ArgumentException("data contains an invalid field: " + t, nameof(data));

                var hdr = t[0..4].ToLower();
                var res = t[4..];
                ...
                {
                    case "byr:":
                        Trace.Assert(string.IsNullOrEmpty(BirthYear));
                        BirthYear = res;
                        break;
                    case "iyr:":
                        Trace.Assert(string.IsNullOrEmpty(IssueYear));
                        IssueYear = res;
                        break;
                    case "eyr:":
                        Trace.Assert(string.IsNullOrEmpty(ExpiryYear));
                        ExpiryYear = res;
                        break;
                    case "hgt:":
                        Trace.Assert(string.IsNullOrEmpty(Height));
                        Height = res;
                        break;
                    case "hcl:":
                        Trace.Assert(string.IsNullOrEmpty(HairColor));
                        HairColor = res;
                        break;
                    case "ecl:":
                        Trace.Assert(string.IsNullOrEmpty(EyeColor));
                        EyeColor = res;
                        break;
                    case "pid:":
                        Trace.Assert(string.IsNullOrEmpty(Id));
                        Id = res;
                        break;
                    case "cid:":
                        Trace.Assert(string.IsNullOrEmpty(Country));
                        Country = res;
                        break;

                    default:
                        throw new ArgumentException("Invalid field in passport data: " + t, nameof(data));
                }
            }
        }

        public static PassportData Parse(in string data)
        {
            Trace.Assert(!string.IsNullOrWhiteSpace(data));

            return new PassportData(data.Split((char[])null, StringSplitOptions.RemoveEmptyEntries));
        }
    }
    ...
    class Program04
    {
        ...
        static bool IsValidPart1(in string passport)
        {
            var pd = PassportData.Parse(passport);
            if (string.IsNullOrEmpty(pd.BirthYear) || string.IsNullOrEmpty(pd.ExpiryYear) ||
                string.IsNullOrEmpty(pd.EyeColor) || string.IsNullOrEmpty(pd.HairColor) ||
                string.IsNullOrEmpty(pd.Height) || string.IsNullOrEmpty(pd.Id) ||
                string.IsNullOrEmpty(pd.IssueYear))
            {
                return false;
            }
            return true;
        ...
        static bool IsValidPart2(in string passport)
        {
            var pd = PassportData.Parse(passport);
            if (string.IsNullOrEmpty(pd.BirthYear) || string.IsNullOrEmpty(pd.ExpiryYear) ||
                string.IsNullOrEmpty(pd.EyeColor) || string.IsNullOrEmpty(pd.HairColor) ||
                string.IsNullOrEmpty(pd.Height) || string.IsNullOrEmpty(pd.Id) ||
                string.IsNullOrEmpty(pd.IssueYear) || pd.Id.Length != 9 || pd.Height.Length < 4 || pd.Height.Length > 5)
            {
                return false;
            }

            int by = int.Parse(pd.BirthYear);
            int iy = int.Parse(pd.IssueYear);
            int ey = int.Parse(pd.ExpiryYear);
            if (by < 1920 || by > 2002 || iy < 2010 || iy > 2020 || ey < 2020 || ey > 2030)
            {
                return false;
            }

            var heightUnit = pd.Height[^2..];
            if (heightUnit != "cm" && heightUnit != "in")
                return false;
            var heightNum = int.Parse(pd.Height[0..^2]);
            switch (heightUnit)
            {
                case "cm":
            ...
            if (pd.HairColor.Length == 7 && pd.HairColor[0] == '#')
            {
                var clr = pd.HairColor[1..];
                foreach (var c in clr)
                {
                    switch (c)
                    {
                        case '0':
         ...
         * passport. While these documents are extremely similar, North Pole Credentials aren't issued by a country and
        ...
        {
            int result = 0;
            foreach (var dat in Data04.example)
            {
                if (IsValidPart1(dat))
                    result++;
            }
            ...
            result = 0;
            foreach (var dat in Data04.actual)
            {
                if (IsValidPart1(dat))
                    result++;
            }
