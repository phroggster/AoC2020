...
namespace Day08
{
    ...
    static class Part1
    {
        ...
        {
            if (Run(Data.Test.Item1) != Data.Test.Item2)
                throw new ApplicationException("Test data was not validated correctly, cannot continue");

            var result = Run(Data.Actual);
            Console.WriteLine(result);
            // Solution: 1501
        }

        static Tuple<OpCode, int> Parse(in string data)
        {
            if (!Enum.TryParse(data[0..3], out OpCode oc))
                oc = OpCode.invalid;

            return new Tuple<OpCode, int>(oc, int.Parse(data[4..]));
        }

        static int Run(in List<string> program)
        {
            int eax = 0;                // Accumulator
            int eip = 0;                // Instruction pointer (current index)
            var esp = new List<int>();  // Stack pointer (visited indices)

            while (!esp.Contains(eip) && eip < program.Count)
            {
                esp.Add(eip);

                var instruction = Parse(program[eip]);
                switch (instruction.Item1)
                {
                    case OpCode.nop:
                        break;
                    case OpCode.acc:
                        eax += instruction.Item2;
                        break;
                    case OpCode.jmp:
                        eip += instruction.Item2;
                        continue;
                    case OpCode.invalid:
                    default:
                        throw new ApplicationException($"Unknown opcode: '{program[eip]}'");
                }
                eip++;
            }
