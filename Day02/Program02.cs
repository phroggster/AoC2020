...
namespace Day02
{
    ...
    static class Part1
    {
        static uint Solve(in List<DataItem> inputs)
        {
            uint result = 0;

            foreach (var item in inputs)
            {
                var count = item.Pass.Count(c => c == item.Tok);
                if (count >= item.A && count <= item.B)
                    result++;
            }
        ...
        {
            Trace.Assert(Solve(Data02.example) == 2);

            var result = Solve(Data02.actual);
    ...
    static class Part2
    {
        static uint Solve(in List<DataItem> inputs)
        {
            ...
            foreach (var item in inputs)
            {
                var a = item.Pass.ElementAt((int)(item.A - 1));
                var b = item.Pass.ElementAt((int)(item.B - 1));
                if ((a == item.Tok || b == item.Tok) && a != b)
                    result++;
            }
        ...
        public static void Solve()
        {
            Trace.Assert(Solve(Data02.example) == 1);
